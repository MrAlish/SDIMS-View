...
const CreateOccupation = ({ clickedIdData }) => {
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = (data) => {
    return new Promise((resolve) => {
      setTimeout(() => {
        try {
          createOccupation(data).then((response) => {
  ...
  return (
    <>
      <CommonHeaderDesign title={"Create Occupation"} />
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <AddButton
          icon={<FaPlus />}
          title={isSubmitting ? "Submitting..." : "Add Occupation"}
