...
const CreateBusinessOwnershipType = ({ clickedIdData }) => {
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = (data) => {
    return new Promise((resolve) => {
      setTimeout(() => {
        try {
          createBusinessOwnershipType(data).then((response) => {
  ...
  return (
    <>
      <CommonHeaderDesign title={"Create Business Ownership Type"} />
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex justify-end ">
          <div className="flex gap-2 bg-blue-400  py-3 shadow-lg rounded-md w-36 justify-center hover:bg-blue-300 cursor-pointer ">
            ...
            >
              {isSubmitting ? "Submitting..." : "Add Business Owner"}
            </button>
          </div>
        </div>
      </form>
...
export default CreateBusinessOwnershipType;
