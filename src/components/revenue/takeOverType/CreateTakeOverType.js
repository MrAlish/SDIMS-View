...
const CreateTakeOverType = ({ clickedIdData }) => {
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = (data) => {
    return new Promise((resolve) => {
      setTimeout(() => {
        try {
          createTakeoverType(data).then((response) => {
  ...
  return (
    <>
      <CommonHeaderDesign title={"Create Take Over Type"} />
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex justify-end ">
          <div className="flex gap-2 bg-blue-400  py-3 shadow-lg rounded-md w-36 justify-center hover:bg-blue-300 cursor-pointer ">
            ...
            >
              {isSubmitting ? "Submitting..." : "Add Take Over"}
            </button>
          </div>
        </div>
      </form>
...
export default CreateTakeOverType;
